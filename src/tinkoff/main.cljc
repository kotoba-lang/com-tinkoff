(ns tinkoff.main
  "Kotodama WASM entrypoint for the Tinkoff clean-room actor (L4) — Clojure port.
  ...
  facts (`tinkoff.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "tinkoff")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"tinkoff.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "tinkoff-compat" :tier tier :entities entities} 200])
...
(defn start! [] :tinkoff-compat/ready)
